...
<template>
	<app-screen-modal @keydown.esc="closePreferencesModal" @click-blend="closePreferencesModal">
		<app-card block :closable="true" @close="closePreferencesModal" title="Preferences">
			<form @submit.prevent="onSubmitPreferences">
				<app-input v-model="location" class="mb-2" name="location" label="Location"></app-input>
				<app-input v-model="bio" class="mb-2" name="bio" label="Bio"></app-input>
				<app-select
					class="mb-4"
					name="theme"
					v-model="theme"
